...
def create_model_directory(model_path, base_dir=None, use_batch_analysis=False, 
                          eval_refinement=None, eval_budget=None, eval_max_level=None,
    ...
    Args:
        ...
        base_dir (str): Base directory (defaults based on use_batch_analysis flag)
        use_batch_analysis (bool): Use batch analysis directory structure
        eval_refinement (int): Evaluation refinement level for config_id
        eval_budget (int): Evaluation element budget for config_id  
        eval_max_level (int): Evaluation max level for config_id
        sweep_name (str): Sweep name for batch analysis structure
        
    Returns:
        str: Path to model-specific directory
    """
    if use_batch_analysis:
        # Use batch analysis directory structure
        if base_dir is None:
            base_dir = os.path.join(PROJECT_ROOT, 'analysis', 'data', 'model_performance', 
                                  sweep_name, 'batch_analysis')
        
        # Build config_id from evaluation parameters
        if eval_refinement and eval_budget and eval_max_level:
            config_id = f"ref{eval_refinement}_budget{eval_budget}_max{eval_max_level}"
            model_output_dir = os.path.join(base_dir, config_id)
        else:
            # Fallback to model directory name if eval parameters not provided
            model_dir_name = Path(model_path).parent.name
            model_output_dir = os.path.join(base_dir, model_dir_name)
            print(f"Warning: Using fallback directory naming. Provide --eval-refinement, --eval-budget, --eval-max-level for proper batch analysis structure.")
    else:
...
def main():
    ...
    parser.add_argument('--batch-analysis-output', action='store_true',
                    help='Save outputs to batch analysis directory structure instead of default animations directory')
    parser.add_argument('--eval-refinement', type=int, 
                        help='Evaluation refinement level (for batch analysis directory naming)')
    parser.add_argument('--eval-budget', type=int,
                        help='Evaluation element budget (for batch analysis directory naming)')
    parser.add_argument('--eval-max-level', type=int,
                        help='Evaluation max level (for batch analysis directory naming)')
    ...
    if args.plot_mode:
        if args.output_dir:
            ...
            args.model_path,
            use_batch_analysis=args.batch_analysis_output,
            eval_refinement=args.eval_refinement,
            eval_budget=args.eval_budget, 
            eval_max_level=args.eval_max_level
        )
    else:
        output_dir = None
