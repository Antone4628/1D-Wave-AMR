"""
Discontinuous Galerkin Wave Solver with Sequential Adaptive Mesh Refinement

This module implements a high-order Discontinuous Galerkin (DG) solver for the 1D wave equation
with sequential h-adaptation capabilities optimized for RL-driven mesh refinement.
...
- Optimized for sequential element-by-element adaptation
"""
...
class DGWaveSolverSequential:
    """
    Discontinuous Galerkin solver for 1D wave equation with Sequential Adaptive Mesh Refinement (AMR).
    
    This solver is optimized for element-by-element mesh adaptation driven by reinforcement learning.
    ...
    def __init__(self, nop, xelem, max_elements, max_level, courant_max=0.1, icase=1, periodic=True, verbose=False, balance=False):
        """
        Initialize the DG wave solver with sequential AMR capabilities.
    ...
    def adapt_single_element(self, element_idx, action, update_solution=True):
        """
        Adapt a single element based on the specified action.
        
        This is optimized for sequential adaptation where elements are processed one by one.
        
        Args:
            element_idx: Index of element in active list
            action: -1 (coarsen), 0 (do nothing), or 1 (refine)
            update_solution: Whether to compute steady-state solution after adaptation
        ...
        try:
            ...
            # Update solution if requested
            if update_solution:
                improved_solution = self.steady_solve_improved()
                self.q = improved_solution
    ...
    def adapt_mesh(self, marks, element_budget=None, balance=None, update_dt=True):
        """
        ...
        if estimated_new_elements > element_budget:
            if self.verbose:
    ...
    def pseudo_step(self, dt=None):
        """
        Take single time step with forcing term.
        
        Args:
            dt (float, optional): Time step size. If None, use the solver's dt
            
        Returns:
            array: Updated solution
        """
        if dt is None:
            dt = self.dt
    ...
    def steady_solve_improved(self):
        ...
        This method provides improved stability and accuracy over the standard method.
        
        Returns:
            array: Steady-state solution
        """
        # Create matrices for non-periodic case
        periodicity_non_periodic = np.arange(self.npoin_dg)
        self.Me = create_mass_matrix(
            self.intma, self.coord, self.nelem, self.ngl, 
            self.nq, self.wnq, self.psi
        ...
        # Identify boundary nodes and set boundary condition
    ...
    def _perform_fixed_refinement(self, target_level):
        ...
        for level in range(target_level):
            ...
            # Apply refinement
            self.adapt_mesh(marks=marks, update_dt=False)
