...
class DGWaveSolverMixed:
    ...
    def _initialize_forcing(self):
        """Initialize the forcing function based on test case."""
        self.f = eff(self.coord, self.npoin_dg, self.icase, self.wave_speed)
        return self.f
    ...
    def steady_solve(self):
        ...
        # self._initialize_forcing()
        # f = self.f
        # print(f'forcing function shape: {np.shape(self.f)}')
        # print(f'forcing function: {(self.f)}')
        # f = get_forcing(self)
        # f = np.zeros(self.npoin_dg)
        # for i in range(self.npoin_dg):
        #     x = self.coord[i]
        #     f[i] = -2 * self.wave_speed * beta * x * np.exp(-beta * x**2)
